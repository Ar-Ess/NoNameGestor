...
float Easing::Ease(float start, float end, float dt)
{
	if (!params.active) return params.mode == EaseMode::IN_MODE ? end : start;
...
double Easing::LinearEaseIn    (double t, double d)
{
	return (t / d);
}
double Easing::LinearEaseOut   (double t, double d)
{
	return 1.0 - (t / d);
}
double Easing::LinearEaseInOut (double t, double d)
{
	return 1.0 - Maths::Abs((2.0 * (t / d)) - 1);
}
...
{
	t /= d;
	return t < 0.5 ? 2 * t * t : 1 - (double)Maths::Pow(-2 * t + 2, 2) / 2;
}
...
{
	t /= d;
	return t < 0.5 ? 4 * t * t * t : 1 - (double)Maths::Pow(-2 * t + 2, 3) / 2;
}
...
{
	t /= d;
	return t < 0.5 ? 8 * t * t * t * t : 1 - (double)Maths::Pow(-2 * t + 2, 4) / 2;
}
double Easing::QuintEaseIn     (double t, double d)
{
	t /= d;
	return  (double)Maths::Pow(t, 5);
}
double Easing::QuintEaseOut    (double t, double d)
{
	t /= d;
	return 1 - (double)Maths::Pow(1 - t, 5);
}
double Easing::QuintEaseInOut  (double t, double d)
{
	t /= d;
	return t < 0.5 ? 16 * t * t * t * t * t : 1 - Maths::Pow(-2 * t + 2, 5) / 2;
}
double Easing::SineEaseIn      (double t, double d) 
{
	return -1 * (double)Maths::Cos(t / d * (Maths::Pi() / 2)) + 1;
}
double Easing::SineEaseOut     (double t, double d) 
{
	return 1 * (double)Maths::Sin(t / d * (Maths::Pi() / 2));
}
...
{
	return Maths::Pow(2, 10 * float(t / d - 1));
}
double Easing::ExpoEaseOut     (double t, double d) 
{
	return (-Maths::Pow(2, -10 * t / d) + 1);
}
double Easing::ExpoEaseInOut   (double t, double d) 
{
	...
	return t == 0
		...
		: t == 1
		? 1
		: t < 0.5 ? Maths::Pow(2, 20 * t - 10) / 2
		: (2 - Maths::Pow(2, -20 * t + 10)) / 2;
}
double Easing::CircEaseIn      (double t, double d) 
{
	t /= d;
	return -1 * ((double)Maths::Sqrt(1 - float(t * t)) - 1);
}
double Easing::CircEaseOut     (double t, double d) 
{
	t /= d;
	t--;
	return Maths::Sqrt(1 - float(t * t));
}
...
double Easing::ElasticEaseIn   (double t, double d) 
{
	...
	double a = 1;
	double s = p / 4;
	return -(a * Maths::Pow(2, 10 * float(t -= 1)) * Maths::Sin((double(t * d) - s) * (2 * Maths::Pi()) / p));
}
double Easing::ElasticEaseOut  (double t, double d) 
{
	...
	double a = 1;
	double s = p / 4;
	return a * Maths::Pow(2, float(-10 * t)) * Maths::Sin((double(t * d) - s) * (2 * Maths::Pi()) / p) + 1;
}
double Easing::ElasticEaseInOut(double t, double d) 
{
	...
	if (t < 1) return -.5f * (a * Maths::Pow(2, 10 * float(t -= 1)) * Maths::Sin((double(t * d) - s) * (2 * Maths::Pi()) / p));

	return a * Maths::Pow(2, -10 * float(t -= 1)) * Maths::Sin((double(t * d) - s) * (2 * Maths::Pi()) / p) * .5f + 1;
}
...
double Easing::GetValue()
{
	...
	case EaseType::LINEAR:
	{
		switch (params.mode)
		{
		case EaseMode::IN_MODE: val = LinearEaseIn(timer, params.duration); break;
		case EaseMode::OUT_MODE: val = LinearEaseOut(timer, params.duration); break;
		case EaseMode::IN_OUT_MODE: val = LinearEaseInOut(timer, params.duration); break;
		}
		break;
	}
