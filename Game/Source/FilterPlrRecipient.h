...
class FilterPlrRecipient : public Recipient
{
public: // Functions

	FilterPlrRecipient(const char* name, float money, bool hidden, bool open, float* totalMoneyPtr) : Recipient(name, money, hidden, open, RecipientType::FILTER_PLURAL)
	{
		this->totalMoneyPtr = totalMoneyPtr;
		NewLabel();
	}

	~FilterPlrRecipient() override
	{
		ClearLabels();
	}
